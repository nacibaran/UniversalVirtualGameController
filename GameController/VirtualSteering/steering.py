...
with mp_hands.Hands(
  ...
  while cap.isOpened():
    ...
    if len(co) == 2:
        ...
        cv2.circle(img=image, center=(int(xm), int(ym)), radius=radius, color=(0, 255, 0), thickness=90)

        l = (int(math.sqrt((co[0][0] - co[1][0]) ** 2 * (co[0][1] - co[1][1]) ** 2)) - 150) // 2
        cv2.line(image, (int(xa), int(ya)), (int(xb), int(yb)), (0, 255, 0), 12)
        if co[0][0] > co[1][0] and co[0][1]>co[1][1] and co[0][1] - co[1][1] > 65:
          ...
          #  cv2.putText(image, "Turn left", (50, 50), font, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
            cv2.line(image, (int(xbp), int(ybp)), (int(xm), int(ym)), (0, 255, 0), 12)
        ...
        #    cv2.putText(image, "Turn right", (50, 50), font, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
            cv2.line(image, (int(xap), int(yap)), (int(xm), int(ym)), (0, 255, 0), 12)
    ...
    cv2.putText(image, "Naci-Baran", (30, 30), font, 0.6, (0, 0, 255), 1, cv2.LINE_AA)
